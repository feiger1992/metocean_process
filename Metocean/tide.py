...
def str_time_from_seconds(seconds): return str(round(divmod(seconds, 3600)[
    0])) + '小时' + str(round(divmod(seconds, 3600)[1] / 60)) + '分钟'


def str_time_from_second2(t): return '%s小时%s分钟' % (t.seconds // 3600, (t.seconds % 3600) // 60) if (
    t.seconds % 3600) % 60 < 30 else '%s小时%s分钟' % (t.seconds // 3600, (t.seconds % 3600) // 60 + 1)
...
class Tide(object):
    ...
    def out_txt(self, site, tide_out):
        def num_txt(num): return str(round(num, 2))
        s = ''
        unit = '米' if (tide_out['dif_max'] < 10) else '厘米'
        s += '潮位观测站' + site + '的平均潮位为' + num_txt(tide_out['mean']) + unit
        s += ',平均高潮位为' + \
            num_txt(tide_out['h_mean']) + unit + ',高潮位最高为' + num_txt(tide_out['h_max']) + unit
        s += ',平均低潮位为' + \
            num_txt(tide_out['l_mean']) + unit + ',低潮位最低为' + num_txt(tide_out['l_min']) + unit
        s += ',平均潮差为' + num_txt(tide_out['dif_mean']) + unit + \
            ',最大潮差为' + num_txt(tide_out['dif_max']) + unit
        s += ',最小潮差为' + num_txt(tide_out['dif_min']) + unit + \
            ',平均涨潮历时为' + str_time_from_second2(tide_out['raise_time'])
    ...
    def preprocess(self, s, threshold=10):
        ...
        num_time = np.linspace(
            0,
            len(t) *
            self.deltatime.total_seconds() /
            3600,
            len(t))
    ...
    def process_one(self, temp_data):
        ...
        temp_data['if_min'] = np.r_[True, temp_data.tide.values[1:] < temp_data.tide.values[:-1]
                                    ] & np.r_[temp_data.tide.values[:-1] < temp_data.tide.values[1:], True]
        temp_data['if_max'] = np.r_[True, temp_data.tide.values[1:] > temp_data.tide.values[:-1]
                                    ] & np.r_[temp_data.tide.values[:-1] > temp_data.tide.values[1:], True]
        ...
        def calculate_time_diff(temp_data):
            for x in temp_data.loc[temp_data.if_min].format_time:
                try:
                    temp_data.loc[x, 'ebb_time'] = x - temp_data[(temp_data.if_max) & (
                        temp_data.format_time < x)].format_time.max()
                    temp_data.loc[x, 'diff'] = temp_data.loc[temp_data[(temp_data.if_max) & (
                        temp_data.format_time < x)].format_time.max(), 'tide'] - temp_data.loc[x, 'tide']
                except BaseException:
                    pass

            for x in temp_data.loc[temp_data.if_max].format_time:
                try:
        ...
        temp_data.loc[temp_data[temp_data['raising_time'] <
                                datetime.timedelta(hours=2)].index, 'if_max'] = False
        temp_data.loc[temp_data[temp_data['raising_time'] <
                                datetime.timedelta(hours=2)].index, 'raising_time'] = np.nan

        temp_data.loc[temp_data[temp_data['ebb_time'] <
                                datetime.timedelta(hours=2)].index, 'diff'] = np.nan
        temp_data.loc[temp_data[temp_data['ebb_time'] <
                                datetime.timedelta(hours=2)].index, 'if_min'] = False
        temp_data.loc[temp_data[temp_data['ebb_time'] <
                                datetime.timedelta(hours=2)].index, 'ebb_time'] = np.nan

        # 根据大小潮个数筛选
        switch = temp_data.loc[temp_data.if_min].append(
            temp_data.loc[temp_data.if_max]).sort_index().index
        count_filtertime = 0
        count = 0
        count2 = 0
        while (len(temp_data.loc[temp_data.if_min]) != len(temp_data.loc[temp_data.if_max])) or (
                count > 20 and abs(
                    len(temp_data.loc[temp_data.if_min]) - len(temp_data.loc[temp_data.if_max])) > 2):
            count += 1
            print('第' + str(count) + '次筛选')
            print('低潮潮位个数' + str(len(temp_data.loc[temp_data.if_min])))
            print('高潮潮位个数' + str(len(temp_data.loc[temp_data.if_max])))

            if ((len(temp_data.loc[temp_data.if_min]) - len(temp_data.loc[temp_data.if_max]) == 1) or (
                len(temp_data.loc[temp_data.if_min]) - len(
                    temp_data.loc[temp_data.if_max]) == -1)) or count < 30:
                print('极值只多一个')
                count2 += 1
            ...
            for i in range(1, len(switch) - 1):
                ...
                if (temp_data.loc[switch[i - 1],
                                  'if_max'] and temp_data.loc[switch[i + 1],
                                                              'if_max']) or (temp_data.loc[switch[i - 1],
                                                                                           'if_min'] and temp_data.loc[switch[i + 1],
                                                                                                                       'if_min']):
                    pass
                else:
    ...
    def harmonic_analysis(self, site, if_init=True, time_zone=8, lat=30):  # 19年9月启用
        ...
        ana_data['UTC_time'] = ana_data['time'] - \
            pandas.Timedelta(time_zone, 'h')
    ...
    def plot_tide_compare(self, site=None):
        ...
        self.tt1 = temp_data.loc[temp_data.if_max].format_time  # 高潮时刻
        self.hh1 = temp_data.loc[temp_data.if_max].tide  # 高潮潮位
        self.tt2 = temp_data.loc[temp_data.if_min].format_time  # 低潮时刻
        self.hh2 = temp_data.loc[temp_data.if_min].tide  # 低潮时间

        line1, = ax.plot(temp_data.format_time, temp_data.tide_init,
                         'o', markersize=1.5, label='原始数据', color='blue')
        line2, = ax.plot(temp_data.format_time, temp_data.tide,
                         linewidth=0.5, color='black', label='去噪以后的数据')

        line3, = ax.plot(self.tt1, self.hh1, marker='*', ls='',
                         markersize=4, color='red', label='高潮时刻')
        line4, = ax.plot(self.tt2, self.hh2, marker='*', ls='',
                         markersize=4, color='green', label='低潮时刻')
...
class Process_Tide(Tide):
    def output(
        ...
        for site in x.data.keys():
            ...
            if max(x.data[site].index) - \
                    min(x.data[site].index) < datetime.timedelta(days=50):
                try:
                    x.month[site] = [
                        ((x.month[site][0]).append(
                            (x.month[site][1]))).sort_index()]
                except BaseException:
                    pass
            ...
            for mon in x.month[site]:
                ...
                o_clock = mon.loc[mon.groupby(
                    mon.index.minute == 0).groups[True], ['tide']]
                ...
                for r in range(row1, 3 + row1):
                    sheet.set_row(r, height=12.5)
                for r in range(row1 + 3, row1 + 41):
                    sheet.set_row(r, height=11.25)

                # 补全星号
                if len(x.month[site]) != 1:
                    for ii in range(row1 + 6,
                                    row1 + 6 + o_clock.index[1].daysinmonth):
                ...
                for i in o_clock.index:
                    ...
                    if len(x.month[site]) != 1:
                        ...
                        hanghanghang = (
                            i.to_pydatetime().date() - o_clock.index[0].to_pydatetime().date()).days + 1
                    if len(x.month[site]) != 1:
                        sheet.write(row1 + 5 + i.day, 0, i.date(), format_date)
                        sheet.write(
                            row1 + 5 + i.day,
                            35,
                            self.solar2lunar_d(i),
                            format_cn)
                        sheet.write(
                            row1 + 5 + i.day, i.hour + 1, h, format_num)
                    else:

                        sheet.write(
                            row1 + 5 + hanghanghang, 0, i.date(), format_date)
                        ...
                        sheet.write(
                            ...
                            format_cn)
                        sheet.write(
                            row1 + 5 + hanghanghang, i.hour + 1, h, format_num)
                    t.append(h)  # 当天的
                    ...
                    if len(x.month[site]) != 1:
                        ...
                        sheet.write(
                            ...
                            format_num)
                        sheet.write(
                            row1 + 6 + i.daysinmonth, 21, '月 合 计 ', format_cn)
                        # row1 + 7 + i.daysinmonth, 24,
                        sheet.write(
                            row1 + 7 + i.daysinmonth, 21, '月 平 均 ', format_cn)
                    else:
                        sheet.write(
                            row1 + 5 + hanghanghang,
                            25,
                        ...
                        sheet.write(
                            ...
                            format_num)
                        sheet.write(
                            row1 + 5 + hangshu + 1, 21, '月 合 计 ', format_cn)
                        sheet.write(
                            row1 + 6 + hangshu + 1, 21, '月 平 均 ', format_cn)
                # if len(x.month[site]) == 1:
                ...
                for ii in range(21):
                    sheet.write(
                        row1 + 8 + hangshu, ii, None, format_cn_a_left1)
                ####################################################高低潮位处理#####
                self.high = mon.loc[mon.groupby(mon.if_max).groups[True], [
                    'tide', 'format_time']]
                high = self.high
                self.low = mon.loc[mon.groupby(mon.if_min).groups[True], [
                    'tide', 'format_time']]
                low = self.low
                t1 = 0
                t_count = []
                ...
                for i in high.dropna().index:
                    ...
                    sheet.write(row1 + 5 + hanghanghang_1,
                                27 + move1, i, format_time)

                    if i == high.tide.idxmax():
                        sheet.write(row1 +
                                    5 +
                                    hanghanghang_1, 28 +
                                    move1, high.loc[i, ['tide']] *
                                    alpha, format_num_r)
                    else:
                        sheet.write(row1 +
                                    5 +
                ...
                sheet.write(row1 + 6 + hangshu, 2, '月 最 高 高 潮 = ' + \
                            str(round(alpha * high.tide.max(), 0)), format_cn_a_left)

                sheet.write(row1 +
                            7 +
                            hangshu, 2, '潮 时 = ' +
                            str(high.tide.idxmax().month) +
                            '月' +
                            str(high.tide.idxmax().day) +
                            '日' +
                            str(high.tide.idxmax().hour) +
                            '时' +
                            str(high.tide.idxmax().minute) +
                            '分', format_cn_a_left)

                self.high = mon.loc[mon.groupby(mon.if_max).groups[True], [
                    'tide', 'format_time', 'raising_time']]
                high = self.high
                for i in high.dropna().index:
                ...
                for i in low.dropna().index:
                    ...
                    sheet.write(row1 + 5 + hanghanghang_2,
                                31 + move1, i, format_time)
                    if i == low.tide.idxmin():
                        sheet.write(row1 +
                                    5 +
                                    hanghanghang_2, 32 +
                                    move1, low.loc[i, 'tide'] *
                                    alpha, format_num_r)
                    else:
                        sheet.write(row1 +
                                    5 +
                                    ...
                                    move1, low.loc[i, 'tide'] *
                                    alpha, format_num)
                sheet.write(row1 + 6 + hangshu, 9, '月 最 低 低 潮 = ' + \
                            str(round(alpha * low.tide.min(), 0)), format_cn_a_left)

                sheet.write(row1 +
                            7 +
                            hangshu, 9, '潮 时 = ' +
                            str(low.tide.idxmin().month) +
                            '月' +
                            str(low.tide.idxmin().day) +
                            '日' +
                            str(low.tide.idxmin().hour) +
                            '时' +
                            str(low.tide.idxmin().minute) +
                            '分', format_cn_a_left)

                self.low = mon.loc[mon.groupby(mon.if_min).groups[True], [
                    'tide', 'format_time', 'ebb_time']]
                low = self.low
                for i in low.dropna().index:
                ...
                sheet.write(
                    ...
                    str(
                        round(
                            alpha *
                            numpy.mean(
                                mon['diff'].dropna().values),
                            0)),
                    format_cn_a_left)
                sheet.write(row1 + 7 + hangshu, 15, '月 最 大 潮 差 = ' + \
                            str(round(mon['diff'].max() * alpha, 0)), format_cn_a_left)
                sheet.write(row1 + 8 + hangshu, 15, '月 最 小 潮 差 = ' + \
                            str(round(mon['diff'].min() * alpha, 0)), format_cn_a_left1)
                ###############################################################
                r_1 = row1 + 7
                r_2 = r_1 + hangshu - 1
                sheet.merge_range(
                ...
                sheet.write(
                    ...
                    16,
                    '观测日期：' +
                    mon.format_time.min().strftime('%Y/%m/%d') +
                    '至' +
                    mon.format_time.max().strftime('%Y/%m/%d'),
                    format_title2)
                sheet.write(
                    row1 +
                    1,
                ...
                for c in range(1, 25):
                    sheet.merge_range(
                        row1 + 3, c, row1 + 5, c, c - 1, format_cn)
    ...
    def display(self, site):
        ...
        dfs = df2.style.bar(
            subset=[
                'tide', 'tide_init', 'diff'], color=[
                '#d65f5f', '#5fba7d'], align='mid')
