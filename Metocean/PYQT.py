...
class M_window(QMainWindow):
    ...
    def display_tide_TXT(self, filename, site, tip):
        ...
        self.display(
            category='TXT',
            title=site,
            content=t[filename].data[site].to_string(
                columns=columns,
    ...
    def display_tide_harmonic_result(self, filename, site, tip):
        self.display(
            category='TXT',
            title=site + '调和分析结果',
            content=t[filename].harmonic_result[site],
            tip=tip)

    def tide_process(self, filename, sitename, threshold):
        preprocess_thread = Thread(
            target=t[filename].preprocess, kwargs={
                's': sitename, 'threshold': threshold})
        preprocess_thread.start()
        second = 0
        while preprocess_thread.is_alive():
            sig.msg_to_show.emit('正在对' +
                                 sitename +
                                 '站点数据进行预处理，用时' +
                                 str(round(second, 1)) +
                                 '秒')
    ...
    def tide_site_element_event(self, file, site):
        self.tides_control.sites_control[site].signal.tide_preprocess.connect(
            self.tide_process)
        self.tides_control.sites_control[site].signal.show_tide_HTML_signal.connect(
            self.display_tide_HTML)
        self.tides_control.sites_control[site].signal.show_tide_TXT_signal.connect(
            self.display_tide_TXT)
        self.tides_control.sites_control[site].signal.show_tide_harmonic_result_signal.connect(
            self.display_tide_harmonic_result)
        self.tides_control.sites_control[site].signal.show_diff_signal.connect(
            self.show_pic)
        self.tides_control.sites_control[site].signal.show_tide_statics_signal.connect(
            self.display)
...
class TideTab(QWidget):
    ...
    def layout_element(self):
        ...
        self.gen_Tieddocument_button = QPushButton(
            '生成潮位报告', clicked=self.genTideDocument2File)
    ...
    def open_file(self):
        ...
        if self.check_multifile.isChecked():
            fileName, _ = QFileDialog.getOpenFileNames(
                self, "选取文件", "", "Excel文件(*.xlsx);;97-03表格文档 (*.xls)", options=options)
        else:
        ...
        for f in fileName:
            try:
                t.update(
                    {f: Process_Tide(f, only_first_sheet=self.if_one_sheet.isChecked())})
        ...
        for filename, process_tide in t.items():
            ...
            for site in process_tide.sites:
                self.sites_control.update(
                    {site: Tide_Site_Tab(site, filename)})
                self.sites_Tab.addTab(self.sites_control[site], site)
                self.sites_Tab.setTabToolTip(
                    self.sites_Tab.count() - 1, filename)
    ...
    def genTideDocument2File(self):
        self.tideDocumentName, _ = QFileDialog.getSaveFileName(
            self, "保存潮位报告文件", "", "Word 文档(*.docx)", options=QFileDialog.Options())
...
class Tide_Site_Tab(QWidget):
    ...
    def layout_element(self):
        ...
        self.confirm_change_altitudinal = QPushButton(
            '改变数据高程', clicked=self.change_altitude)
        ...
        self.Show_tide_data_button1 = QPushButton(
            "&潮位查看\n（WEB模式）", clicked=self.show_tide_HTML_click)

        self.Show_tide_data_button2 = QPushButton(
            "&潮位查看\n（普通模式）", clicked=self.show_tide_TXT_click)

        self.Show_tide_data_button3 = QPushButton(
        ...
        self.Save_Mid_Data_Button = QPushButton(
            "保存中间结果", clicked=self.save_one_site_mid_data)
        self.Astronomical_Tide_Analysis = QPushButton(
            "天文潮计算", clicked=self.Astronomical_analysis)
        ...
        self.document_out_button = QPushButton(
            "输出文字统计", clicked=self.show_statics_click)
    ...
    def preprocess_click(self):
        self.signal.tide_preprocess.emit(
            self.filename,
            self.sitename,
    ...
    def show_tide_HTML_click(self):
        ...
        self.signal.show_tide_HTML_signal.emit(
            self.filename, self.sitename, '处理阀值为：' + str(self.Process_threshold.value()))
    ...
    def show_tide_TXT_click(self):
        self.signal.show_tide_TXT_signal.emit(
            self.filename, self.sitename, '处理阀值为：' + str(self.Process_threshold.value()))
    ...
    def generate_tide_HTML(self):
        ...
        while html_process_thread.is_alive():
            sig.msg_to_show.emit('正在生成潮位对应HTML文件，处理用时' +
                                 str(round(second, 1)) + '秒')
            time.sleep(0.1)
            second += 0.1
        sig.msg_to_show.emit(
            self.filename +
    ...
    def show_statics_click(self):
        self.signal.show_tide_statics_signal.emit('TXT',
                                                  self.sitename + '潮位特征值',
    ...
    def save_something_to_excel(self, fun, args, file_type_in_chinese):
        ...
        savefileName, _ = QFileDialog.getSaveFileName(
            self, "保存" + file_type_in_chinese, "", "Excel文件 (*.xlsx)", options=options)
        if savefileName:
            ...
            while save_tide_thread.is_alive():
                sig.msg_to_show.emit('正在生成' +
                                     self.sitename +
                                     file_type_in_chinese +
                                     '，已用时' +
                                     str(round(second, 1)) +
                                     '秒，请稍等')
                time.sleep(0.1)
    ...
    def save_one_site_tide(self):
        self.save_something_to_excel(t[self.filename].output,
                                     {'sitename': self.sitename,
                                      ...
                                      'reference_system': self.Altitudinal_System.combobox.currentText()},
                                     '潮位报表')

    def change_altitude(self):
        t[self.filename].change_altitude(
            self.sitename, diff=self.Altitudinal_Add.value())
    ...
    def Astronomical_analysis(self):
        ...
        if self.analysis_by_init_data.isChecked():
            ...
            s = '去噪后的数据'
        self.signal.show_tide_harmonic_result_signal.emit(
            self.filename, self.sitename, s)
...
class Altitudinal_Select(QDialog):
    ...
    def setText(self):
        ...
        text, ok = QInputDialog.getText(
            self, "自定义高程系统", "自定义名称:", QLineEdit.Normal, self.combobox.itemText(3))
        if ok and text != '':
            self.select = text
...
class Current_Tab(QWidget):
    ...
    def bed_group_Layout(self):
        ...
        self.save_bedded_file = QPushButton(
            '分层并保存结果', clicked=self.save_bedded_file_click)
    ...
    def layout_element(self):
        ...
        self.save_Point_and_Tidetype_count = QPushButton(
            '确认测点、潮型', clicked=self.confirm_point)
        self.import_from_report = QPushButton(
            '从潮流报表导入', clicked=self.import_from_report_click)
        ...
        l_group_layout.addRow(self.all_Tide_type, self.ceng_select)
        l_group_layout.addRow(
            self.save_Point_and_Tidetype_count,
            self.import_from_report)
        l_group_layout.addRow(QLabel("编辑测点名称"), self.change_Point_name)
        l_group_layout.addRow(
            self.gen_all_duration_Button,
            self.draw_current_Button)
        l_group_layout.addRow(
            self.save_mean_out_Button,
            self.save_max_out_Button)
        l_group_layout.addRow(
    ...
    def gen_all_current_out(self):
        ...
        try:
            if len(c) != self.Point_count.value() * \
                    len(self.Tide_types) or not c:
                ...
                self.all_current_out = pandas.concat(
                    self.list_of_all_current_out)
    ...
    def ave_out(self):
        ...
        def out_ave_txt_c(row):
            return str_from_df(row['层数']) + '的平均流速为' + num_from_df(
                row['平均流速']) + 'cm/s,平均流向为' + num_from_df(row['平均流向']) + '°,'
    ...
    def gen_all_duration_click(self):
        self.gen_all_duration_out()
        self.sig.show_all_out_signal.emit(
            'TXT', '涨落潮历时汇总', pandas.Series(
                self.dic_duration).to_string())

    def save_csv(self, df):
        options = QFileDialog.Options()
        filename, _ = QFileDialog.getSaveFileName(
            self, "保存所有点位特征值文件", '', "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=options)
        if filename:
            self.filename = filename
            if "csv" in filename:
                df.to_csv(filename)
    ...
    def save_max_out(self):
        self.gen_all_current_out()
        self.all_current_out['最大流速对应方向'] = pandas.to_numeric(
            self.all_current_out['最大流速对应方向'])
        self.all_current_out['最大流速'] = pandas.to_numeric(
            self.all_current_out['最大流速'])
        self.max_out = pandas.pivot_table(self.all_current_out,
                                          values=['最大流速',
                                                  '最大流速对应方向'],
                                          index=['Point',
                                                 '潮型',
                                                 '涨落潮'],
                                          columns=['层数']).swaplevel(0,
                                                                    1,
                                                                    axis=1).reindex(['垂线平均',
                                                                                     '表层',
                                                                                     '0.2H',
    ...
    def save_mean_out(self):
        self.gen_all_current_out()
        self.all_current_out['平均流速'] = pandas.to_numeric(
            self.all_current_out['平均流速'])
        self.all_current_out['平均流向'] = pandas.to_numeric(
            self.all_current_out['平均流向'])
        self.mean_out = pandas.pivot_table(self.all_current_out,
                                           values=['平均流速',
                                                   '平均流向'],
                                           index=['Point',
                                                  '潮型',
    ...
    def process_excel_data(self, c_data):
        ...
        for _, i in c_data.data.items():
            for _, ii in i.items():
                ii.preprocess()
                sig.msg_to_show.emit(ii.tide_type + ii.point + '处理结束')
                print(ii.tide_type + ii.point + '处理结束')

        """
                one_thread = Process_excel_data_thread(ii)
                threads.append(one_thread)
        sig.msg_to_show.emit(str(len(threads)) + '线程加载结束，开始处理数据')
        count_threads = 1
        for one in threads:
            print('*'* 10 ,end= '   ' + str(count_threads))
            print(str(one))
            print('*' * 10, end= '\n')
            one.start()
    ...
    def import_from_report_click(self):
        ...
        for P_name in c_excel.points:
            try:
                ang = self.set_Angles_of_excel_data(P_name, ang)
            except BaseException:
        ...
        for Point_Name, Contral in self.Point_control.items():
            for tide in c_excel.tide_type:
                Contral.One_tide_file_open(
                    is_format_reprot=True,
                    reportFile=self.current_report_file,
                    tideType=tide,
                    angle=Excel_angles[Point_Name],
                    data=c_excel.data[Point_Name][tide])

        sig.msg_to_show.emit('潮流报表数据处理结束')
        print('ok')

    def set_Angles_of_excel_data(self, Point_Name, ang=0):
        d_angle, ok = QInputDialog.getDouble(
            self, "请输入" + Point_Name + "主流向", '落潮流向为正值，涨潮流向为负值', ang, -360, 360, 2)
    ...
    def gen_ALL_fig(self):
        # 检查数据完整
        for Point in self.Point_control:
            ts = self.Point_control[Point]
            for tide in ts.tides_control:
                ts.tides_control[tide].show_current_arrow()
                print(Point, end='点，潮汛：')
                print(tide, end="图像输出完成\n")
    ...
    def Point_control_creat(self, Point):
        one_point_tab = Point_Tab(
            Point,
            tide_types=self.Tide_types,
            cengshu=self.ceng_count)
    ...
    def open_bed_file(self):
        try:
            ...
            pass
        self.pre_bed_fileName, _ = QFileDialog.getOpenFileName(
            self, "选取待分层文件", "", "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=self.options)
        if self.pre_bed_fileName:
            self.beding_current = Current_pre_process(
                filename=self.pre_bed_fileName, is_VD=self.if_vd.isChecked())
            sig.msg_to_show.emit('文件载入成功')

    def save_bedded_file_click(self):
        fileName, _ = QFileDialog.getSaveFileName(
            self, '保存分层文件路径', self.pre_bed_fileName, "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=self.options)
        if fileName:
            self.beding_current.all_fenceng(
                bin=self.bin_height.value(),
                first_bin=self.first_bin.value(),
                top_ratio=self.top_ratio.value(),
                button_ratio=self.button_ratio.value(),
                reverse=self.if_reverse.isChecked())
            self.beding_current.save_result(
                outfile=fileName,
                V_D=self.if_save_VD,
    ...
    def open_current_file(self):
        fileName, _ = QFileDialog.getOpenFileName(
            self, "选取文件", '', "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=self.options)
        if fileName:
            self.filename = fileName
            return fileName
    ...
    def vd_to_en(self):
        options = QFileDialog.Options()
        vd_fileName, _ = QFileDialog.getOpenFileName(
            self, "选取流速流向格式潮流文件", "", "逗号分割文档(utf-8) (*.csv);;Excel文件(*.xlsx)", options=options)
        ne_fileName, _ = QFileDialog.getSaveFileName(
            self, "保存为分向流速格式潮流文件", "", "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=options)
        try:
    ...
    def ne_to_vd(self):
        options = QFileDialog.Options()
        ne_fileName, _ = QFileDialog.getOpenFileName(
            self, "选取分向流速格式潮流文件", "", "逗号分割文档(utf-8) (*.csv);;Excel文件(*.xlsx)", options=options)
        vd_fileName, _ = QFileDialog.getSaveFileName(self, "保存为流速流向格式潮流文件", dir_of_file(
            ne_fileName), "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=options)
        try:
    ...
    def select_positions_file(self):
        self.position_file, _ = QFileDialog.getOpenFileName(
            self, "选取位置文件", "", "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=QFileDialog.Options())
    ...
    def draw_current_cad(self):
        ...
        def plot_line2(x, y, vs, ds, tuceng, parameter, drawing):
            end_x, end_y = end_point(x, y, vs, ds, parameter)
            line = dxf.line((x, y), end_point(x, y, vs, ds, parameter))
            line_a1 = dxf.line(end_point(x, y, vs, ds, parameter), end_point(
                end_x, end_y, vs / 10, dir_in_360(ds - 190), parameter))
            line_a2 = dxf.line(
                end_point(
                    x,
                    y,
            ...
            line_a1['layer'] = tuceng
            drawing.add(line_a1)

            line_a2['layer'] = tuceng
            drawing.add(line_a2)
        ...
        try:
            ...
            try:
                ...
                return 0
        """
        self.cad_file,_ = QFileDialog.getSaveFileName(self,"选取cad各层流速矢量图文件保存路径","","DXF文档(*.dxf)")
        try:
        ...
        drawing.save()
        QMessageBox.information(self,'通知',"分层流速矢量图绘制完成")
        """
        # 再画一个逐时分层的cad矢量图
        self.cad_file, _ = QFileDialog.getSaveFileName(
            self, "选取cad逐时流速矢量图文件保存路径", "", "DXF文档(*.dxf)")
        try:
            drawing = dxf.drawing(self.cad_file)
        except BaseException:
            return 0
...
class Wind_Tab(QWidget):
    ...
    def layout_element(self):
        self.wind_l_Group = QGroupBox("载入文件")
        self.load_file_button = QPushButton(
            '载入数据', clicked=self.preprocess_excel)
        self.load_file_button.setFixedHeight(50)
        self.if_multi_dir = QCheckBox('只有一列方向数据')
        self.if_multi_dir.setChecked(True)

        self.wind_l_Group_layout = QVBoxLayout()
        self.wind_l_Group_layout.addWidget(self.load_file_button)
        self.wind_l_Group_layout.addWidget(self.if_multi_dir)

        self.wind_l_Group.setLayout(self.wind_l_Group_layout)

        self.manipulation_Group = QGroupBox('处理风/浪数据')

        mainLayout = QHBoxLayout()
        mainLayout.addWidget(self.wind_l_Group, stretch=4)
        mainLayout.addWidget(self.manipulation_Group, stretch=20)

        self.setLayout(mainLayout)

    def manipulation_element(self):
        self.manipulation_Group_layout = QFormLayout()
        self.select_column = QComboBox()
        self.select_column.addItems(self.wind_or_wave.items)

        self.select_out_dir_button = QPushButton(
            "选择输出路径", clicked=self.select_out_dir)

        self.manipulation_Group_layout.addRow("选择所处理数据项", self.select_column)
        self.manipulation_Group_layout.addRow(self.select_out_dir_button)

        self.manipulation_Group.setLayout(self.manipulation_Group_layout)

    def select_out_dir(self):
        options = QFileDialog.DontResolveSymlinks | QFileDialog.ShowDirsOnly
        directory = QFileDialog.getExistingDirectory(self,
                                                     "选择生成文件所在路径",
                                                     "XXXXX", options=options)
        if directory:
            self.wind_or_wave.set_save_dir(directory)
            print("生成文件路径已设置成为" + directory)

    def preprocess_excel(self):
        excel_file, _ = QFileDialog.getOpenFileName(
            self, '波浪或气象文件', "", "Excel文件(*.xlsx)", options=self.options)
        if not excel_file:
            return 0
        self.wind_or_wave = Wind_Wave(
            filename=excel_file,
            if_multiple=not self.if_multi_dir.isChecked())
        self.manipulation_element()
        print("载入文件成功，文件来源为" + excel_file)
...
class Point_Tab(QWidget):
    ...
    def layout_element(self):
        ...
        self.load_Spring_file = QPushButton(
            '载入大潮文件', clicked=self.Spring_file_open)
        self.load_Mid_file = QPushButton('载入中潮文件', clicked=self.Mid_file_open)
        self.load_Neap_file = QPushButton(
            '载入小潮文件', clicked=self.Neap_file_open)
    ...
    def One_tide_file_open(
            self,
            is_format_reprot=False,
            ZhangorLuo=False,
            reportFile=None,
            tideType=None,
            angle=None,
        ...
        if not is_format_reprot:
            options = QFileDialog.Options()
            fileName, _ = QFileDialog.getOpenFileName(
                self, "选取文件", self.filename[:-8], "Excel文件(*.xlsx);;逗号分割文档(utf-8)(*.csv)", options=options)
            if fileName:
                self.filename = fileName
                ...
                def generate_tab(tide_type, **kwargs):
                    ...
                    tab = Single_Tide_tab(
                        ...
                        zhang_or_luo=self.zhang_or_luo.isChecked(),
                        Point=self.point,
            ...
            tab = Single_Tide_tab(
                ...
                tide_type=tideType,
                angle=angle,
                zhang_or_luo=ZhangorLuo,
                Point=self.point,
                cengshu=self.cengshu,
                is_format_reprot=is_format_reprot,
                data=data)
...
class Single_Tide_tab(QWidget):  # 需根据从报表导入重构
    def __init__(
        ...
        if is_format_reprot:
            ...
            c.update(
                {
                    self.Point +
                    self.tide_type: Single_Tide_Point(
                        filename=self.filename,
                        tide_type=tide_type,
                        point=Point,
                        angle=angle,
                        zhang_or_luo=zhang_or_luo,
                        cengshu=cengshu)})
            preprocess_thread = Thread(
                target=c[self.Point + self.tide_type].preprocess)
            second = 0
            preprocess_thread.start()
            while preprocess_thread.is_alive():
                sig.msg_to_show.emit('正在对' +
    ...
    def layout_element(self):
        self.output_Button = QPushButton(
            "输出潮流特征值", clicked=self.show_current_proper_TXT_click)
    ...
    def show_current_proper_TXT_click(self):
        ...
        self.signal.show_current_proper_TXT_signal.emit(
