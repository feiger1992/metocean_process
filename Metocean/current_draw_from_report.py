...
def aoe(fun, v_es, v_ns):
    ...
    return vs
class Read_Report():
    def __init__(self, filename = r"C:\2019-沈家湾三期（GK-2019-0118水）\实测数据\附录C：走航流速流向报表 - 流矢图.xlsx"):
        ...
        def format_df(df):
            ...
            try:
                ...
                df.columns = ['depth', 'v_0', 'd_0', 'v_2', 'd_2', 'v_4', 'd_4', 'v_6', 'd_6', 'v_8', 'd_8', 'v_10',
                              'd_10', 'v', 'd']
            df = df[['depth', 'v_0', 'd_0', 'v_2', 'd_2', 'v_4', 'd_4', 'v_6', 'd_6', 'v_8', 'd_8', 'v_10',
                              'd_10', ]]

            df['time'] = df.index
            if (df['v_0'] == 0).all():
                return df.reindex(
                    ['time', 'depth',  'v_2', 'd_2', 'v_6', 'd_6', 'v_8', 'd_8'], axis=1).reset_index()
            else:
                return df.reindex(
        ...
        def draw_rose(ii):
            ...
            ax.bar(d, v, normed=True, opening=0.8, edgecolor='white', bins=np.linspace(0, 200, 11), N=10,
                   cmap=cm.rainbow)
            # ax.box(dir, var, normed=True, edgecolor='white', bins=bins,  cmap=cm.rainbow)
            ax.set_legend(title=item + unit if unit else item, fancybox=True, facecolor='ivory',
                          edgecolor='black',
                          fontsize=15, bbox_to_anchor=(0, 0), decimal_places=0, ncol=3, prop={'size': 9})
...
def Draw_tide_Profile(Single_time,Tide_type,temp_tide_file = r"C:\2019-沈家湾三期（GK-2019-0118水）\报告撰写\潮流对应时间潮位（程序读取）.xlsx"):
...
def Draw_Profile_Current(e,n,Tide_type,Single_Time,Profile_Name):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(len(Profiles[Profile_Name])/2 + 1, 8))
    ...
    ax.yaxis.grid(True)
    plt.xticks(range(1,len(Profiles[Profile_Name]) + 1),Profiles[Profile_Name], fontsize=10)
    plt.yticks(range(8), ['', '底层', '0.8H', '0.6H', '0.4H', '0.2H', '表层', '垂线平均'], fontsize=10)
        ...
        for i in a.ALL_DATA:
            if i.Point in Profiles[Profile_Name] and i.Tide_type == Tide_type:
                ...
                for c in [0,2,4,6,8,10]:
                    THREE_CELLS = False
                    try:
                        i2.Data['e_' + str(c)] = aoe(east,i.Data['v_' + str(c)],i.Data['d_' + str(c)])
                        i2.Data['n_' + str(c)] = aoe(north,i.Data['v_' + str(c)],i.Data['d_' + str(c)])
                    except:
                        i2.Data['e_' + str(c)] = 0
                        i2.Data['n_' + str(c)] = 0
                        THREE_CELLS = True
                ...
                if  THREE_CELLS:
                    i2.Data['e'] = (i2.Data['e_2']  + i2.Data['e_6']  + i2.Data['e_8']) / 3
                    i2.Data['n'] = (i2.Data['n_2']  + i2.Data['n_6']  + i2.Data['n_8'] ) / 3
                else:
                    i2.Data['e'] = (i2.Data['e_0'] + i2.Data['e_2'] * 2 + i2.Data['e_4'] * 2 + i2.Data['e_6'] * 2 + i2.Data['e_8'] * 2 + i2.Data['e_10']) / 10
                    i2.Data['n'] = (i2.Data['n_0'] + i2.Data['n_2'] * 2 + i2.Data['n_4'] * 2 + i2.Data['n_6'] * 2 + i2.Data['n_8'] * 2 + i2.Data['n_10']) / 10
        ...
        for Single_Time in times:
            ...
            for p in Tide_Profile_Data:
                ...
                for c in ['','_0','_2','_4','_6','_8','_10']:
                    es.append(p.Data.loc[p.Data.time == Single_Time,'e' + str(c)].values[0])
                    ns.append(p.Data.loc[p.Data.time == Single_Time,'n' + str(c)].values[0])
